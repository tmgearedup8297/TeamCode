...
public class CryptoboxDetector extends OpenCVPipeline {
    ...
    public int getWidth() {
        return workingMat.width();
    }

    public int getHeight() {
        return workingMat.height();
    }


    public void setDetectionMode(String color){
        if(color.equalsIgnoreCase("RED"))
            detectionMode = CryptoboxDetectionMode.RED;
        else
            detectionMode = CryptoboxDetectionMode.BLUE;
    }
    @Override
    ...
    public double getDistancePos1(int col) {
        int center = this.getWidth();
        double midpointoffset = center - this.getCryptoBoxCenterPosition() - 3;
        if (col == 1) {
            return midpointoffset * pxToInch *-1;
        } else if (col == 0) {
            return ((this.getCryptoBoxCenterPosition() - this.getCryptoBoxLeftPosition() - midpointoffset) * pxToInch);
        } else
            return ((this.getCryptoBoxCenterPosition() - this.getCryptoBoxRightPosition() + midpointoffset) * pxToInch);
    }
